AddCSLuaFile("ent_stepbox_big.lua")
...
function ENT:PhysicsCollide(data, phys)
	...
	elseif data.HitEntity:GetClass() == "ent_stepbox_big" and  data.HitEntity.Built == true then
...
function ENT:FinishBuild()
	...
	local function DestroyAndReturn()
		...
		if IsValid( self.CreatorSwep ) then
			self.CreatorSwep:SetClip1( self.CreatorSwep:Clip1() + 5 )
		end
	end
	
			
	--check once if any player is in its space, if so, destroy itself to free the player
	local mins = self:GetPos() + Vector(-94,-94,-94)
	local maxs = self:GetPos() + Vector(94,94,94) 
	...
	for k, ent in pairs( orgin_ents ) do
		...
		--if a big stepbox is in the way   (they cannot be built on top of each other)	
		elseif ent:GetClass() == "ent_stepbox_big" and ent != self then
			DestroyAndReturn()
			return
